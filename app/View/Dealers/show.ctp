...
<table id="content">
    <tr>
        ...
        <td id="main">
            <?php if(!empty($data['Dealer']['dealer_id']) && !empty($data['Dealer']['dealer_message'])){?>
                <div style="border:1px solid #5175c9;background-color:#e6e6e6;padding:5px;margin-bottom: 10px;">
                    <h3><?php echo $data['Dealer']['dealer_subject'];?></h3>
                    <p><?php echo $data['Dealer']['dealer_message'];?></p>
                    <?php echo $this->Html->link('Clear message', '/dealers/clear_dealer_message/'.$id, array('style' => 'font-size:7pt;'));?>
                </div>
            <?php }?>
            <?php
            ...
            ?>
                    ...
                    <table>
                        ...
                        $fields = array(
                            ...
                            'email' => array(),
                            'phone' => array('label' => 'Phone 1', 'dealer_limit' => TRUE),
                            'phone2' => array('label' => 'Phone 2', 'dealer_limit' => TRUE),
                            'fax' => array('dealer_limit' => TRUE),
                            ...
                            'alternate_truckload_name' => array('label' => 'Alternate Truckload Name', 'dealer_limit' => TRUE),
                            'alternate_truckload_address' => array('label' => 'Alternate Truckload Address', 'dealer_limit' => TRUE),
                            'alternate_truckload_city' => array('label' => 'Alternate Truckload City', 'dealer_limit' => TRUE),
                            'alternate_truckload_state' => array('label' => 'Alternate Truckload State', 'options' => $stateAbbrs, 'dealer_limit' => TRUE),
                            'alternate_truckload_zip' => array('label' => 'Alternate Truckload Zip', 'dealer_limit' => TRUE),
                            ...
                            if(($admin == 1 && (!isset($data['Dealer']['dealer_id']) || empty($data['Dealer']['dealer_id']))) || !isset($field['dealer_limit'])){
                                ...
                                <?php }elseif(isset($field['wysiwyg'])){
                                    ...
                                    <tr>
                                        ...
                                        <td class="form-data">
                                            <?php if(isset($field['before']) && !empty($field['before'])){
                                                echo $field['before'];
                                            }?>
                                            <?php echo $this->Wysiwyg->textarea($model.'.'.$field_name, $field, $wysiwyg_options);?>
                                            <?php if(isset($field['after']) && !empty($field['after'])){
                                                echo $field['after'];
                                            }?>
                                        </td>
                                    </tr>
                    ...
                    <?php }else{?>
                        ...
                        <?php if(!empty($data['Dealer']['dealer_id'])){?>
                            <div class="form-bottom">
                                ...
                                <p>
                                    Rejection Message: <i>(This is only sent to the dealer if you reject the changes and will not be saved)</i><br />
                                    <?php echo $this->Form->textarea('Message.rejection_message', array('style' => 'width:514px'));?><br />
...
<script language="JavaScript" type="text/javascript">
    ...
    function rejectChanges(){
    	$("form-dealers").action = "<?php echo $this->Html->url('/dealers/reject_dealer_changes/'.$data['Dealer']['id']); ?>";
        $('input-action').value = "reject"; //changing hidden field id=input-action
        $('form-dealers').submit(); //submit form-dealers
