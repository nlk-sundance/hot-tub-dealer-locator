                    ...
                    switch ($attribs['type'])
                    {
                        ...
                        case 'pending':
                            if($this->data['Filter'][$name] == 'N'){
                                ...
                                $query .= " AND DealersPending.id IS NOT NULL AND DealersPending.approval_ready = 1";
                            }
                            break;
                    }
                }
 ...
 */
            ...
            if ($this->Dealer->saveAll($save_data)){
                ...
                if($data['Save']['action'] == "approve"){
                    if(isset($data['Message']['approval_message'])){
                        $data['Message']['approval_message'] = trim($data['Message']['approval_message']);
                        if(!empty($data['Message']['approval_message'])){
                            $this->email_dealer_feedback($id, $data['Message']['approval_message'], 1);
                        }
                    }
        ...
        $this->set('stateAbbrs', $this->State->find('list', array('sort' => 'position ASC', 'fields' => array('abbreviation', 'abbreviation'), 'conditions' => array('not' => array('abbreviation' => '')))));
        $this->set('id', $id);
        if(!empty($id)){
    ...
    function clear_dealer_message($id){
        $dealer_id = $this->Dealer->field('dealer_id', array('id' => $id));
        $this->Dealer->create();
        $this->Dealer->id = $dealer_id;
        $this->Dealer->saveField('dealer_message', '');
        $this->Dealer->saveField('dealer_subject', '');
    ...
    function reject_dealer_changes($id){
        $group_id = $this->checkLogin($id);
        if(isset($this->data['Message']['rejection_message'])){
            $message = trim($this->data['Message']['rejection_message']);
            if(!empty($message)){
                $this->email_dealer_feedback($id, $message, 0);
            }
        }
    ...
    function email_dealer_feedback($id, $feedback, $approved = 1)
    {
        ...
        $this->Dealer->saveField('dealer_subject', $subject);
    ...
    function email_update_received($id)
    {
        $email = '';
        if($this->Session->check("login")){
            $user = $this->Session->read("login");
        ...
        if(!empty($email)){
            ...
            $subject = 'Updated Content Received';    

            $message = '<html><head><title>' . $subject . '</title></head>';
            $message .= '<body><p>Thank you for submitting your updated content for your Sundance Spas dealership page. We appreciate your dedication to providing consumers with the with the latest information. We will review content and get back to you within 2 business days.</p>';
            $message .= '</body></html>';
            $headers = 'MIME-Version: 1.0' . "\r\n";
            //$headers .= 'Cc: chris.barnwell@ninthlink.com' . "\r\n";
            $headers .= 'From: dealers@ninthlink.com' . "\r\n" .
    ...
    function find_dealers(){
        ...
        if(!empty($this->data)){
            $zip_array = array();
            $delimiters = array(' ,', ', ', ',', "\n", "\r\n");
            $zips = str_replace($delimiters, $delimiters[0], $this->data['Dealer']['zips']);
            $zips = explode($delimiters[0], $zips);
            ...
            foreach($zips as $z){
                ...
                $is_CA = FALSE;
                if (!preg_match("/"."^\d{5}([\-]?\d{4})?$"."/i",$z)){
                    $is_CA = TRUE;
                }
                $zip_array[$z] = $this->Zipcode->getZipCode($z, $is_CA);
                if($is_CA && !empty($zip_array[$z])){
                    $zip_array[$z] = $zip_array[$z][0];
                }
                if(!empty($zip_array[$z])){
                    $zip_array[$z]['Dealer'] = $this->get_nearest_dealer($zip_array[$z]['Zipcode']['zipcode'], $is_CA);
                }
            }
            
            $this->set('zip_array', $zip_array);
        }
    }
    
    function get_nearest_dealer($zip, $is_CA){
        $road_wiggle = 1.625;
        $temp = $this->Dealer->query("SELECT Dealer.name, Dealer.id,
            $road_wiggle * ( 3963 *
        ...
        FROM dealers Dealer, zipcodes, zipcodes AS zipcodesearch
        WHERE zipcodesearch.zipcode = '" .addslashes($zip). "'
        AND Dealer.zip = zipcodes.zipcode
        AND Dealer.published = 'Y'
        AND Dealer.dealer_id IS NULL
        AND Dealer.country_id = '". ($is_CA ? 3 : 1) ."' 
