...
class LocatorsController extends AppController
{
    ...
    function index(){
        ...
        $this->set('stateList', $this->State->find('all', array('conditions' => array('not' => array('State.name' => array("", "DC")), 'State.country_id' => 1), 'fields' => array('name', 'abbreviation'), 'order' => array('State.name ASC'), 'recursive' => -1))); #generate list of states
        $this->set('provList', $this->State->find('all', array('conditions' => array('not' => array('State.name' => array("", "DC")), 'State.country_id' => 3), 'fields' => array('name', 'abbreviation'), 'order' => array('State.name ASC'), 'recursive' => -1))); #generate list of states
    ...
    function show($subDomain, $country=null, $zip=null){
        ...
        #country other than US or CA
            ...
            $owns = $this->Dealer->query("SELECT Dealer.address1, Dealer.name, Dealer.id, Dealer.slug,
                                                        ...
                                                        AND Dealer.published = 'Y'
                                                        AND Dealer.dealer_id IS NULL
                                                        GROUP BY Dealer.address1 ORDER BY Dealer.name");
        }
        return $owns;
